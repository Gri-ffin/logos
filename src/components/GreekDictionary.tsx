      ...
      try {
        ...
        setDefinitionHtml(
          data.definition_html ||
            'No definition found in Ancient Greek section.'
        )
      } catch (err: any) {
  ...
  return (
    <div className='my-10 w-full font-sans'>
      ...
      <h3 className='text-3xl font-extrabold text-slate-900 border-b-4 border-indigo-500 pb-3 mb-6'>
        ...
        <strong className='text-indigo-600 font-serif italic text-4xl'>
          {lemma}
        </strong>
      </h3>

      {loading && (
        <p className='text-slate-600'>Fetching Wiktionary definition...</p>
      )}

      {error && (
        <div className='bg-red-100 text-red-700 p-4 rounded-lg text-base border border-red-300'>
          {error}
        </div>
      )}
